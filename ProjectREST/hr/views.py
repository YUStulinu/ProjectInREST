...
def console_view(request):
    console_category = Category.objects.get(category='Consola')
    console_products = Product.objects.filter(category=console_category).order_by('id')
    context = {'consoles': console_products}
...
def search(request):
    ...
    if query:
        ...
        for keyword in keywords:
            s = Search().filter('term', description=keyword)
            response = s.execute()
    ...
    return render(request, 'searchresults.html', {'product': intersected_product, 'query': query})
